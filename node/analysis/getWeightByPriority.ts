...
export default getCachedFn(getWeightByPriority, { getCacheKey, fileName: 'getWeightByPriority' });

function getWeightsByPriority() {
...
function getCacheKey(priority: number) {
  return hashValues({
    files: [__filename],
    values: [NUM_CARDS, ITERATIONS, CARD_SELECTION_OPTIONS, CARD_SELECTION_PICKS],
  });
}

(() => {
  const s = Date.now();
  const r = getWeightsByPriority();
  const e = Date.now();
  console.log(e - s, r);
})();
