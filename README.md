...
- track card -> weight = 0
- iteratively play games with random priority lists
...
- given a random priority list
- starting from priority 0
- test all to 0.1 accuracy (400 tests for 0.05 accuracy at 95% confidence)
- for each test, lock p0, randomize the other candidates
- remove all that are less than (best - 0.05 \* 2)
- repeat for priority 0...X until nothing is being removed (because lower priorities have less effect)
- for each test, lock pX, randomize p0...X from the pool of remaining candidates, randomize the other candidates
- repeat starting a priority 0 again for accuracy 2x accuracy (0.025 accuracy = x4 tests = 1,600 tests)
- repeat starting a priority 0 again for accuracy 0.005 (40,000 tests)
- lock in the best result, discarding the others
- cache (order -> winRate, maxWinRate, minWinRate)
- this is assuming a normal distribution, see https://en.wikipedia.org/wiki/Checking_whether_a_coin_is_fair for more details

Idea 3 (combination of previous ideas)

- track card -> weight, # games played
- iteratively play games with random priority lists influenced by card weight
- card weight is nudged up or down by a random value of decreasing influence
- after each game, add (win) or subtract (loss) priority weight to each card
- final order is a sorted list of weights
- run for 40k iterations?
...
- run simulations to calculate results
- cache based on source code
...
function E(P: number) {
  // return (1 - Math.pow((T - P - 1) / (T - P), S)) _ Math.pow((T - P) / T, S);
  // return binomialDistribution(S, N, 1 / T);
  return T _ (1 - Math.pow((T - 1) / T, S));
}
```

T = 10, S = 3, N = 3, P = 0 : 0.3
1/10 + 1/10 + 1/10 = SUM(1/T, 1, N)

T = 10, S = 2, N = 1, P = 0 : 0.19
1 - (9/10)^2 = 0.19
1 - ((T - 1)/T)^S

T = 10, S = 2, N = 1, P = 1 : 0.17
100 possibilities = T*T
10 + 9 = 19 possibilities of c = 0
9 + 8 = 17 possibilities of c = 1
(1 - (1 - Math.pow(9/10, 2))) * (1 - Math.pow(8/9, 2)) = 0.17
E(P) = (1 - E(P-1)) \* (1 - ((T - P - 1)/(T - P)))^S

T = 10, S = 2, N = 1, P = 2 : 0.15
100 possibilities = T*T
10 + 9 = 19 possibilities of c = 0
9 + 8 = 17 possibilities of c = 1
17 + 19 = 36 possibilities of c < 2
8 + 7 = 15 possibilities of c = 2
E(P) = (1-((T-P-1)/(T-P))^S) * ((T-P)/T)^S

T = 10, S = 3, N = 1, P = 0 : 0.271

T = 10, S = 3, N = 2, P = 0 : 0.298
1000 possibilities = T*T*T
1 = 3 1's
9 + 9 + 9 = 27 = 2 1's

T = 10, S = 3, N = 1, P = 1 : 0.217

T = 10, S = 3, N = 2, P = 1 : 0.292
100 possibilities = T\*T
